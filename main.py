...
async def command(interaction):
    embed=discord.Embed(title="Info", description="I am a bot made and programmed by @ExplodeCode and I will only work when ExplodeCode turns me on", color=0x00ff33)
...
async def command(interaction, query:app_commands.Range[str, 1]):
    ...
    for i, link in enumerate(links):
        ...
        embed.add_field(name=link.get('href'), value=link.text, inline=False)
        print(result_str)
